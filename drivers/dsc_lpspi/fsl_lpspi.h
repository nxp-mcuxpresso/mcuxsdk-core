...
/*!
 * @brief Calculates and configures the delay based on the desired delay input in nanoseconds.
 *
 * This function calculates the delay scaler value and configures one of the following delays:
 * - SCK to PCS delay (kLPSPI_PcsToSck)
 * - PCS to SCK delay (kLPSPI_LastSckToPcs)
 * - Between transfer delay (kLPSPI_BetweenTransfer)
 *
 * The delay names are available in type lpspi_delay_type_t.
 *
 * The function takes the desired delay value in nanoseconds and calculates the appropriate
 * scaler value based on the current TCR prescaler setting and source clock frequency.
 * The actual delay is calculated as:
 * - For PCS to SCK and SCK to PCS: delayTime = (scaler + 1) * (1 / (srcClock_Hz / PRESCALE))
 * - For Between Transfer: delayTime = (scaler + 2) * (1 / (srcClock_Hz / PRESCALE))
 *
 * The function returns the actual calculated delay in nanoseconds, which may differ from
 * the requested delay due to:
 * - Rounding up to the nearest achievable delay (never goes below requested value)
 * - Clamping to maximum scaler value (255) if requested delay exceeds hardware capability
 * - Setting to minimum delay (scaler = 0) if requested delay is too small
 *
 * Note:
 * - The LPSPI module must be configured for master mode before calling this function
 * - The LPSPI module must be disabled before changing delay values
 * - The delay configuration uses the current TCR prescaler value
 *
 * @param base LPSPI peripheral address.
 * @param delayTimeInNanoSec The desired delay value in nanoseconds.
 * @param whichDelay The delay type to configure (must be of type lpspi_delay_type_t).
 * @param srcClock_Hz Module source input clock frequency in Hertz.
 * @return The actual calculated delay value in nanoseconds.
 */
