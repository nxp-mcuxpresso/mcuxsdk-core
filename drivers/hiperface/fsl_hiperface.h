...
#define DSL_SetPosReadyMode(base, v)  		do {(base)->SYS_CTRL &= ~HIPERFACE_SYS_CTRL_PRDY_MASK;    (base)->SYS_CTRL |= HIPERFACE_SYS_CTRL_PRDY(v);} while (0)
#define DSL_SetSpiPipeActivate(base, v)  	do {(base)->SYS_CTRL &= ~HIPERFACE_SYS_CTRL_SPPE_MASK;    (base)->SYS_CTRL |= HIPERFACE_SYS_CTRL_SPPE(v);} while (0)
#define DSL_SetPolaritySyncPulse(base, v)  	do {(base)->SYS_CTRL &= ~HIPERFACE_SYS_CTRL_SPOL_MASK;    (base)->SYS_CTRL |= HIPERFACE_SYS_CTRL_SPOL(v);} while (0)
#define DSL_SetOutputActivate(base, v)  	do {(base)->SYS_CTRL &= ~HIPERFACE_SYS_CTRL_OEN_MASK;    (base)->SYS_CTRL |= HIPERFACE_SYS_CTRL_OEN(v);} while (0)

#define DSL_GetQualityMonitoringValue(base)			HIPERFACE_MASTER_QM_QM((base)->MASTER_QM)
#define DSL_GetQualityMonitoringLink(base) 			HIPERFACE_MASTER_QM_LINK((base)->MASTER_QM)

#define DSL_GetEventInterruptStatus(base)		HIPERFACE_EVENT_H_INT((base)->EVENT_H)
#define DSL_GetEventSlaveEventSum(base)			HIPERFACE_EVENT_H_SUM((base)->EVENT_H)
#define DSL_GetEventeEstimatorOn(base)			HIPERFACE_EVENT_H_POS((base)->EVENT_H)
#define DSL_GetEventEstimatorThresholdErr(base)	HIPERFACE_EVENT_H_DTE((base)->EVENT_H)
#define DSL_GetEventProtocolRstWarning(base)	HIPERFACE_EVENT_H_PRST((base)->EVENT_H)

#define DSl_GetEventMsgInitStatus(base) 		HIPERFACE_EVENT_L_MIN((base)->EVENT_L)
#define DSl_GetEventLongMsgAnswerErr(base) 		HIPERFACE_EVENT_L_ANS((base)->EVENT_L)
#define DSl_GetEventQMLowValueWarning(base) 	HIPERFACE_EVENT_L_QMLW((base)->EVENT_L)
#define DSl_GetEventLongMsgChannelfree(base) 	HIPERFACE_EVENT_L_FREL((base)->EVENT_L)
...
#define DSL_GetEventMaskMSUM(flag)      HIPERFACE_EVENT_MASK_H_MSUM(flag)
#define DSL_GetEventMaskMPOS(flag)      HIPERFACE_EVENT_MASK_H_MPOS(flag)
#define DSL_GetEventMaskMDTE(flag)      HIPERFACE_EVENT_MASK_H_MDTE(flag)
#define DSL_GetEventMaskMPRST(flag)     HIPERFACE_EVENT_MASK_H_MPRST(flag)

#define DSL_GetEventMaskMFREL(flag)      HIPERFACE_EVENT_MASK_L_MFREL(flag)
#define DSL_GetEventMaskMQMLW(flag)      HIPERFACE_EVENT_MASK_L_MQMLW(flag)
#define DSL_GetEventMaskMANS(flag)       HIPERFACE_EVENT_MASK_L_MANS(flag)
#define DSL_GetEventMaskMMIN(flag)       HIPERFACE_EVENT_MASK_L_MMIN(flag)
...
#define DSL_GetPrimEdges(base)				HIPERFACE_EDGES_PRIM_EDGES((base)->EDGES_PRIM)
#define DSL_GetPrimCableDelay(base)			HIPERFACE_DELAY_PRIM_CABLE_DELAY((base)->DELAY_PRIM)
#define DSL_GetPrimSignalStrength(base)		HIPERFACE_DELAY_PRIM_RSSI((base)->DELAY_PRIM)


#define DSL_GetSafeEdges(base)				HIPERFACE_EDGES_SAFE_EDGES((base)->EDGES_SAFE)
#define DSL_GetSafeCableDelay(base)			HIPERFACE_DELAY_SAFE_CABLE_DELAY((base)->DELAY_SAFE)
#define DSL_GetSafeSignalStrength(base)		HIPERFACE_DELAY_SAFE_RSSI((base)->DELAY_SAFE)


#define DSL_IRQ_AllEventDisable(base) 		do {base->EVENT_MASK_H = 0;base->EVENT_MASK_L = 0;} while(0)

#define DSL_GetPrimVersionMinorRelease(base)  	HIPERFACE_VERSION_PRIM_MINOR_RELEASE((base)->VERSION_PRIM)
#define DSL_GetPrimVersionMajorRelease(base)  	HIPERFACE_VERSION_PRIM_MAJOR_RELEASE((base)->VERSION_PRIM)
#define DSL_GetPrimVersionCoding(base) 			HIPERFACE_VERSION_PRIM_CODING((base)->VERSION_PRIM)

#define DSL_GetSafeVersionMinorRelease(base) 	HIPERFACE_VERSION_SAFE_MINOR_RELEASE((base)->VERSION_SAFE)
#define DSL_GetSafeVersionMajorRelease(base) 	HIPERFACE_VERSION_SAFE_MAJOR_RELEASE((base)->VERSION_SAFE)
#define DSL_GetSafeVersionCoding(base) 			HIPERFACE_VERSION_SAFE_CODING((base)->VERSION_PRIM)


#define DSL_GetSecVersionMinorRelease(base) 	HIPERFACE_VERSION_SEC_MINOR_RELEASE((base)->VERSION_SEC)
#define DSL_GetSecVersionMajorRelease(base) 	HIPERFACE_VERSION_SEC_MAJOR_RELEASE((base)->VERSION_SEC)
#define DSL_GetSecVersionCoding(base) 			HIPERFACE_VERSION_SEC_CODING((base)->VERSION_SEC)
...
//#define DSL_GetStatusSUM0(base)          	DSL_GET_REGISTER_FEILD(base->SUMMARY, SUMMARY, SUM0)

#define DSL_SetLmsgRead(base)         			(base)->PC_ADD_H |= HIPERFACE_PC_ADD_H_LRW_MASK
#define DSL_SetLmsgWrite(base)         			(base)->PC_ADD_H &= ~HIPERFACE_PC_ADD_H_LRW_MASK

#define DSL_SetLmsgAddrWithOffset(base)			(base)->PC_ADD_H |= HIPERFACE_PC_ADD_H_LOFF_MASK
#define DSL_SetLmsgAddrWithoutOffset(base)		(base)->PC_ADD_H &= ~HIPERFACE_PC_ADD_H_LOFF_MASK

#define DSL_GetLmsgLastCauseError(base)			HIPERFACE_PC_ADD_H_LOFF((base)->PC_ADD_H)

#define DSL_GetLmsgLastStatus(base)				HIPERFACE_PC_ADD_H_LOFF((base)->PC_ADD_H)

#define DSL_SetLmsgAddrIndirect(base)			(base)->PC_ADD_H |= HIPERFACE_PC_ADD_H_LIND_MASK
#define DSL_SetLmsgAddrDirect(base)				(base)->PC_ADD_H &= ~HIPERFACE_PC_ADD_H_LIND_MASK

#define DSL_SetLmsgDataLen(base, len)			do {(base)->PC_ADD_H &= ~HIPERFACE_PC_ADD_H_LLEN_MASK;  (base)->PC_ADD_H |= HIPERFACE_PC_ADD_H_LLEN(len);} while(0)

#define DSL_SetLmsgAddrOffset(base, offset)  do { (base)->PC_OFF_H = HIPERFACE_PC_OFF_H_LID_MASK | (((offset) >> 8) & HIPERFACE_PC_OFF_H_LOFFADD_MASK); \
...
#define DSL_SetLmsgAddr(base, addr) 	do {(base)->PC_ADD_H &= ~0x3; \
											(base)->PC_ADD_H |=	HIPERFACE_PC_ADD_H_LADD(addr >> 8); \
