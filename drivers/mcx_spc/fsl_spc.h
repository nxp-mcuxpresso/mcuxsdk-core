...
enum
{
    ...
    kStatus_SPC_CORELDOVoltageDetectWrong = MAKE_STATUS(kStatusGroup_SPC, 9U),    /*!< Settings of CORE_LDO voltage
                                                                                    detection is not allowed. */
    kStatus_SPC_DCDCCoreLdoVoltageMisMatch = MAKE_STATUS(kStatusGroup_SPC, 9U),    /*!< Target voltage level of DCDC
                                                                                    not equal to CORE_LDO. */
};
 ...
 *
 * @note Enumerations may not suitable for all devices, please check the specific device's RM for 
...
enum _spc_analog_module_control
{
    ...
    kSPC_controlAllModules = 0x770F73UL,  /*!< Enable/disable all modules in active or low-power modes. */
};
...
{
    kSPC_SleepWithSYSClockRunning     = 0x0U, /*!< Power domain request SLEEP mode with SYS clock running. */
    kSPC_DeepSleepWithSysClockOff     = 0x1U, /*!< Power domain request deep sleep mode with system clock off. */
    kSPC_PowerDownWithSysClockOff     = 0x2U, /*!< Power domain request power down mode with system clock off. */
    kSPC_DeepPowerDownWithSysClockOff = 0x8U, /*!< Power domain request deep power down mode with system clock off. */
 ...
 * @param base SPC peripheral base address.
 *
 * @return The last low-power mode that the power domain requested.
 */
...
#if (defined(FSL_FEATURE_MCX_SPC_ACTIVE_REG_HAS_VDD_VD_DISABLE_BIT) && FSL_FEATURE_MCX_SPC_ACTIVE_REG_HAS_VDD_VD_DISABLE_BIT)
/*!
 * @brief Masks all LVDs and HVDs when changing the active voltage levels.
 * 
 * @param base SPC peripheral base address.
 */
static inline void SPC_MaskActiveModeVoltageDetections(SPC_Type *base)
{
    base->ACTIVE_CFG &= ~SPC_ACTIVE_CFG_VDD_VD_DISABLE_MASK;
}

/*!
 * @brief Un-masks all LVDs and HVDs when change the active voltage levels.
 * 
 * @param base SPC peripheral base address.
 */
static inline void SPC_UnMaskActiveModeVoltageDetections(SPC_Type *base)
{
    base->ACTIVE_CFG |= SPC_ACTIVE_CFG_VDD_VD_DISABLE_MASK;
}
#endif /* FSL_FEATURE_MCX_SPC_ACTIVE_REG_HAS_VDD_VD_DISABLE_BIT */
