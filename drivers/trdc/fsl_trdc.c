...
static const uint32_t trdc_mbc_slave_increments[3U] = {0x140UL, 0x28UL, 0x28UL};
...
void TRDC_GetHardwareConfig(TRDC_Type *base, trdc_hardware_config_t *config)
{
    ...
    config->masterNumber = (uint8_t)((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMSTR_MASK) >> TRDC_TRDC_HWCFG0_NMSTR_SHIFT);
    config->domainNumber = (uint8_t)((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NDID_MASK) >> TRDC_TRDC_HWCFG0_NDID_SHIFT);
    config->mbcNumber    = (uint8_t)((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMBC_MASK) >> TRDC_TRDC_HWCFG0_NMBC_SHIFT);
    config->mrcNumber    = (uint8_t)((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMRC_MASK) >> TRDC_TRDC_HWCFG0_NMRC_SHIFT);
}
...
{
    assert(NULL != config);
    assert(mbcIdx < (uint8_t)((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMBC_MASK) >> TRDC_TRDC_HWCFG0_NMBC_SHIFT));
    assert(slvIdx < 4U);

    config->blockNum  = base->MBC_INDEX[mbcIdx].MBC_MEM_GLBCFG[slvIdx] & TRDC_MBC_MEM_GLBCFG_NBLKS_MASK;
    config->blockSize = (base->MBC_INDEX[mbcIdx].MBC_MEM_GLBCFG[slvIdx] & TRDC_MBC_MEM_GLBCFG_SIZE_LOG2_MASK) >>
...
#endif
{
    ...
    assert(domainAssignment->domainId <
           ((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NDID_MASK) >> TRDC_TRDC_HWCFG0_NDID_SHIFT));
...
void TRDC_SetIDAU(TRDC_Type *base, const trdc_idau_config_t *idauConfiguration)
{
    ...
    base->TRDC_IDAU_CR = pid._u32 | TRDC_TRDC_IDAU_CR_VLD_MASK;
}
...
{
    assert(NULL != flwConfiguration);
    assert((base->TRDC_FLW_CTL & TRDC_TRDC_FLW_CTL_LK_MASK) == 0U);
...
status_t TRDC_GetAndClearFirstSpecificDomainError(TRDC_Type *base, trdc_domain_error_t *error, uint8_t domainId)
{
    ...
    errorBitMap = base->TRDC_DERRLOC[domainId];
    ...
    else
    {
        ...
        base->TRDC_FDID = TRDC_TRDC_FDID_FDID(domainId);
        ...
        if (errorIndex > 15U)
        {
            ...
            regW1          = base->MRC_DERR[errorIndex].W1;
            error->address = base->MRC_DERR[errorIndex].W0;
            /* Clear error pending. */
            base->MRC_DERR[errorIndex].W3 = TRDC_W3_RECR(0x01U);
        }
        else
        {
            ...
            if (errorIndex >= ((base->TRDC_HWCFG0 & TRDC_TRDC_HWCFG0_NMBC_MASK) >> TRDC_TRDC_HWCFG0_NMBC_SHIFT))
            {
                return kStatus_Fail;
            }
            ...
            /* Get the error information. */
            regW1          = base->MBC_DERR[errorIndex].W1;
            error->address = base->MBC_DERR[errorIndex].W0;
            /* Clear error pending. */
            base->MBC_DERR[errorIndex].W3 = TRDC_W3_RECR(0x01U);
        }
                                   ...
                                   uint8_t regIdx)
{
    ...
    assert((base->MRC_INDEX[mrcIdx].MRC_GLBAC[regIdx] & TRDC_MRC_GLBAC_LK_MASK) == 0U);

    trdc_reg32_convert_t pid;

    pid._memory_access_control                = *config;
    base->MRC_INDEX[mrcIdx].MRC_GLBAC[regIdx] = pid._u32;
}
...
void TRDC_MrcRegionNseSet(TRDC_Type *base, uint8_t mrcIdx, uint16_t regionMask)
{
    ...
    base->MRC_INDEX[mrcIdx].MRC_NSE_RGN_SET = ((uint32_t)regionMask);
}
...
void TRDC_MrcRegionNseClear(TRDC_Type *base, uint8_t mrcIdx, uint16_t regionMask)
{
    ...
    base->MRC_INDEX[mrcIdx].MRC_NSE_RGN_CLR = ((uint32_t)regionMask);
}
...
void TRDC_MrcSetRegionDescriptorConfig(TRDC_Type *base, const trdc_mrc_region_descriptor_config_t *config)
{
    ...
    uint32_t regAddr = (uint32_t) & (base->MRC_INDEX[config->mrcIdx].MRC_DOM0_RGD_W[config->regionIdx][0]);
...
void TRDC_MbcSetNseUpdateConfig(TRDC_Type *base, const trdc_mbc_nse_update_config_t *config, uint8_t mbcIdx)
{
    ...
    base->MBC_INDEX[mbcIdx].MBC_NSE_BLK_INDEX = pid._u32;
}
...
void TRDC_MbcWordNseSet(TRDC_Type *base, uint8_t mbcIdx, uint32_t bitMask)
{
    ...
    base->MBC_INDEX[mbcIdx].MBC_NSE_BLK_SET = ((uint32_t)bitMask);
}
...
void TRDC_MbcWordNseClear(TRDC_Type *base, uint8_t mbcIdx, uint32_t bitMask)
{
    ...
    base->MBC_INDEX[mbcIdx].MBC_NSE_BLK_CLR = ((uint32_t)bitMask);
}
...
void TRDC_MbcSetMemoryBlockConfig(TRDC_Type *base, const trdc_mbc_memory_block_config_t *config)
{
    ...
    for (uint8_t i = 0; i < config->slaveMemoryIdx; i++)
    {
        slaveIncrement += trdc_mbc_slave_increments[i];
    }

    regAddr += (TRDC_MBC_DOMAIN_INCREMENT * config->domainIdx + slaveIncrement) +
