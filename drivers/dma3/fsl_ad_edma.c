...
#define FSL_COMPONENT_ID "platform.drivers.ad_dma3"
#endif
 ...
 */
static uint32_t EDMA_AD_GetInstance(DMA_AD_Type *base);
...
static DMA_AD_Type *const s_edmaBases[] = DMA_AD_BASE_PTRS;
...
static const IRQn_Type s_edmaIRQNumber[][FSL_FEATURE_EDMA_MODULE_CHANNEL] = DMA_AD_IRQS;
...
/* If there is address offset, convert the address */
    ...
    EDMA_AD_SetTransferConfig(handle->base, channel, config, nextTcd);
}
...
#if !(defined(FSL_SDK_DISABLE_DRIVER_CLOCK_CONTROL) && FSL_SDK_DISABLE_DRIVER_CLOCK_CONTROL)
#if (defined(FSL_FEATURE_EDMA_HAS_COMMON_CLOCK_GATE) && (FSL_FEATURE_EDMA_HAS_COMMON_CLOCK_GATE))
    CLOCK_EnableClock(s_edmaClockName[EDMA_AD_GetInstance(base) + 1U]);
#endif /* FSL_FEATURE_EDMA_HAS_COMMON_CLOCK_GATE */
...
void EDMA_AD_Deinit(DMA_AD_Type *base)
{
    ...
    CLOCK_DisableClock(s_edmaClockName[EDMA_AD_GetInstance(base)]);
#endif /* FSL_FEATURE_EDMA_HAS_COMMON_CLOCK_GATE */
#endif /* FSL_SDK_DISABLE_DRIVER_CLOCK_CONTROL */
 ...
 */
void EDMA_AD_SetTransferConfig(DMA_AD_Type *base,
                            uint32_t channel,
                            const edma_transfer_config_t *config,
                            edma_tcd_t *nextTcd)
{
    ...
    EDMA_AD_TcdSetTransferConfig((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), config, nextTcd);
}
...
void EDMA_AD_SetMinorOffsetConfig(DMA_AD_Type *base, uint32_t channel, const edma_minor_offset_config_t *config)
{
    ...
    EDMA_AD_TcdSetMinorOffsetConfig((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), config);
}
...
void EDMA_AD_SetChannelLink(DMA_AD_Type *base, uint32_t channel, edma_channel_link_type_t type, uint32_t linkedChannel)
{
    ...
    EDMA_AD_TcdSetChannelLink((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), type, linkedChannel);
}
...
void EDMA_AD_SetModulo(DMA_AD_Type *base, uint32_t channel, edma_modulo_t srcModulo, edma_modulo_t destModulo)
{
    ...
    EDMA_AD_TcdSetModulo((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), srcModulo, destModulo);
}
...
void EDMA_AD_EnableChannelInterrupts(DMA_AD_Type *base, uint32_t channel, uint32_t mask)
{
    ...
    {
        EDMA_AD_TcdEnableInterrupts((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), mask);
    }
}
...
void EDMA_AD_DisableChannelInterrupts(DMA_AD_Type *base, uint32_t channel, uint32_t mask)
{
    ...
    {
        EDMA_AD_TcdDisableInterrupts((edma_tcd_t *)((uint32_t)&base->CH[channel] + 0x00000020U), mask);
    }
}
...
#endif
        /*
            Before call EDMA_AD_TcdSetTransferConfig or EDMA_AD_SetTransferConfig,
            user must call EDMA_AD_TcdReset or EDMA_AD_ResetChannel which will set
...
void EDMA_AD_CreateHandle(edma_handle_t *handle, DMA_AD_Type *base, uint32_t channel)
{
    ...
    edmaInstance               = EDMA_AD_GetInstance(base);
 ...
 */
void EDMA_AD_PrepareTransferConfig(edma_transfer_config_t *config,
                                void *srcAddr,
                                uint32_t srcWidth,
                                int16_t srcOffset,
                                void *destAddr,
 ...
 */
void EDMA_AD_PrepareTransfer(edma_transfer_config_t *config,
                          void *srcAddr,
                          uint32_t srcWidth,
                          void *destAddr,
                          uint32_t destWidth,
                          ...
                          edma_transfer_type_t transferType)
{
    ...
    EDMA_AD_PrepareTransferConfig(config, srcAddr, srcWidth, srcOffset, destAddr, destWidth, destOffset, bytesEachRequest,
                               transferBytes);
}
...
status_t EDMA_AD_SubmitTransfer(edma_handle_t *handle, const edma_transfer_config_t *config)
{
    ...
    if (handle->tcdPool == NULL)
    {
        ...
        */
        if (EDMA_AD_GetRemainingMajorLoopCount(handle->base, handle->channel) != 0U)
        {
    ...
    else /* Use the TCD queue. */
    {
        ...
        EDMA_AD_TcdReset(&handle->tcdPool[currentTcd]);

        EDMA_AD_TcdSetTransferConfig(&handle->tcdPool[currentTcd], config, NULL);
        /* Enable major interrupt */
...
*/
         ...
         */
        EDMA_AD_InstallTCD(handle->base, handle->channel, &handle->tcdPool[currentTcd]);
...
void DMA3_DriverIRQHandler(uint32_t instance, uint32_t channel)
{
    ...
    if ((EDMA_AD_GetChannelStatusFlags(s_edmaBases[instance], channel) & (uint32_t)kEDMA_InterruptFlag) != 0U)
    {
        EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + channel]);
    }

    SDK_ISR_EXIT_BARRIER;
}

#if defined(FSL_FEATURE_EDMA_MODULE_CHANNEL_IRQ_ENTRY_SUPPORT_PARAMETER) && \
    (FSL_FEATURE_EDMA_MODULE_CHANNEL_IRQ_ENTRY_SUPPORT_PARAMETER)
void DMA2_DriverIRQHandler(uint32_t channel);
void DMA2_DriverIRQHandler(uint32_t channel)
{
    uint32_t instance    = EDMA_AD_GetInstance(DMA2);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;

    if ((EDMA_AD_GetChannelStatusFlags(DMA2, channel) & (uint32_t)kEDMA_InterruptFlag) != 0U)
    {
        EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + channel]);
    }
...
void DMA0_DriverIRQHandler(void)
{
    ...
    uint32_t i = 0;
    uint32_t instance = EDMA_AD_GetInstance(DMA0);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;
    ...
    for (i = 0; i < channel; i++)
    {
        ...
        {
            if ((EDMA_AD_GetChannelStatusFlags(DMA0, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
    }
    SDK_ISR_EXIT_BARRIER;
...
void ADMA_EDMA0_INT_DriverIRQHandler(void)
{
    uint32_t channel = FSL_FEATURE_EDMA_MODULE_CHANNEL_IRQ_ENTRY_SUPPORT_PARAMETERNNEL;
    uint32_t i = 0;
    DMA_AD_Type *base = ADMA__EDMA0;
    uint32_t instance = EDMA_AD_GetInstance(base);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;
    ...
    for (i = 0; i < channel; i++)
    {
        if (s_EDMAEnabledChannel[instance][i])
        {
            if ((EDMA_AD_GetChannelStatusFlags(base, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
...
void AUDIO_EDMA0_INT_DriverIRQHandler(void)
{
    ...
    for (i = 0; i < channel; i++)
    {
        if (s_EDMAEnabledChannel[instance][i])
        {
            if ((EDMA_AD_GetChannelStatusFlags(AUDIO__EDMA0, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
...
void AUDIO_EDMA1_INT_DriverIRQHandler(void)
{
    ...
    for (i = 0; i < channel; i++)
    {
        if (s_EDMAEnabledChannel[instance][i])
        {
            if ((EDMA_AD_GetChannelStatusFlags(AUDIO__EDMA1, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
...
#ifdef DMA__EDMA0
void DMA_AD_EDMA0_INT_DriverIRQHandler(void);
void DMA_AD_EDMA0_INT_DriverIRQHandler(void)
{
    uint32_t channel = FSL_FEATURE_EDMA_MODULE_CHANNEL;
    uint32_t i = 0;
    DMA_AD_Type *base = DMA__EDMA0;
    uint32_t instance = EDMA_AD_GetInstance(base);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;
    ...
    for (i = 0; i < channel; i++)
    {
        if (s_EDMAEnabledChannel[instance][i])
        {
            if ((EDMA_AD_GetChannelStatusFlags(DMA__EDMA0, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
...
#ifdef DMA__EDMA1
void DMA_AD_EDMA1_INT_DriverIRQHandler(void);
void DMA_AD_EDMA1_INT_DriverIRQHandler(void)
{
    uint32_t channel = FSL_FEATURE_EDMA_MODULE_CHANNEL;
    uint32_t i = 0;
    DMA_AD_Type *base = DMA__EDMA1;
    uint32_t instance = EDMA_AD_GetInstance(base);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;
    ...
    for (i = 0; i < channel; i++)
    {
        if (s_EDMAEnabledChannel[instance][i])
        {
            if ((EDMA_AD_GetChannelStatusFlags(DMA__EDMA1, i) & (uint32_t)kEDMA_InterruptFlag) != 0U)
            {
                EDMA_AD_HandleIRQ(s_EDMAHandle[prevChannel + i]);
            }
        }
...
void CONNECTIVITY__EDMA_INT_DriverIRQHandler(void)
{
    uint32_t channel = FSL_FEATURE_EDMA_MODULE_CHANNEL;
    uint32_t i = 0;
    DMA_AD_Type *base = CONNECTIVITY__EDMA;
    uint32_t instance = EDMA_AD_GetInstance(base);
    uint32_t prevChannel = (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL * instance;
