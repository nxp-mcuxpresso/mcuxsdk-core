...
MODULE_YML_PATH = PurePath('mcux/module.yml')
# Path to the blobs folder
MODULE_BLOBS_PATH = PurePath('mcux/blobs')
BLOB_PRESENT = 'A'
BLOB_NOT_PRESENT = 'D'
BLOB_OUTDATED = 'M'
    ...
    if cmake_extern:
        return('\"{}\":\"{}\":\"{}\"\n'
               .format(meta['name'],
                       ...
                       "${MCUX_" + meta['name-sanitized'].upper() + "_CMAKE_DIR}"))
...
def kconfig_snippet(meta, path, kconfig_file=None, blobs=False, sysbuild=False):
    ...
    snippet = [f'menu "{name} ({path.as_posix()})"',
               ...
               else f'osource "$(MCUX_{name_sanitized.upper()}_KCONFIG)"',
               f'config MCUX_COMPONENT_{name_sanitized.upper()}_MODULE',
               '	bool',
               '	default y',
               'endmenu\n']
...
def process_kconfig(module, meta):
    ...
    if os.path.isfile(kconfig_file):
        ...
        name_sanitized = meta['name-sanitized']
        return (f'config MCUX_COMPONENT_{name_sanitized.upper()}_MODULE\n'
                f'   bool\n'
                f'   default y\n')
...
def process_meta(mcux_base, west_projs, modules, extra_modules=None,
                 propagate_state=False):
    # Process mcux_base, projects, and modules and create a dictionary
...
#
# Modules may be set via MCUX_MODULES, MCUX_EXTRA_MODULES,
...
''')
    ...
    if args.meta_out:
        meta = process_meta(args.mcux_base, west_projs, modules,
