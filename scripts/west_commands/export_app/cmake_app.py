...
from .misc import LICENSE_HEAD, AppType, SharedOptions, AppOptions
from .misc import replace_cmake_variables, process_path, should_skip_entry
...
class CmakeApp(object):
    def __init__(self, shared_options: SharedOptions, app_options: AppOptions):
        '''
        Args:
            shared_options (SharedOptions): The shared options for all apps
            app_options (AppOptions): The app options
        '''
        self.shared_options = shared_options
        self.options = app_options
        self.source_dir = self.options.source_dir
        self.is_sysbuild = True if 'sysbuild.cmake' in os.listdir(self.source_dir) else False
        ...
        self.source_list_file = self.options.source_dir / 'CMakeLists.txt'
        self.dest_list_file = self.output_dir / 'CMakeLists.txt'
        self.options.cmake_variables.update({
    ...
    def run(self):
        ...
        try:
            ...
            new_cmake_content = self.parse_cmake_file(self.source_list_file)
    ...
    def process_misc_files(self):
        if self.options.app_type == AppType.main_app:
            example_yml, custom_conf_files, extra_files, replacements = self.process_example_yml(self.shared_options.target_apps)
            if self.is_sysbuild and (sysbuild_confs := self.source_dir / 'sysbuild').exists() and sysbuild_confs.is_dir():
                shutil.copytree(sysbuild_confs, self.output_dir / 'sysbuild')
        else:
            example_yml, custom_conf_files, extra_files, replacements = self.process_example_yml()
        open(self.output_dir / 'example.yml', 'w').write(yaml.dump(example_yml))
        for idx, conf_file in enumerate(custom_conf_files):
    ...
    def process_example_yml(self, target_apps=[]):
        ...
        extra_files = None
        replacements = None
        example_yml = yaml.load(open(self.source_dir / 'example.yml'), yaml.BaseLoader)

        for idx, (k, v) in enumerate(example_yml.items()):
            if target_apps and k not in target_apps:
                continue
            example_yml[k]['section-type'] = 'freestanding_application'
            if idx == 0:
                self.app_name = k
                example_info = example_yml[k]
                extra_files = example_info.get('contents', {}).get('extra_files', [])
                replacements = example_info.get('contents', {}).get('replacements', [])
                if extra_files:
                    del example_info['contents']['extra_files']
                if replacements:
                    del example_info['contents']['replacements']
                continue
            ...
            for ext_arg in extra_build_args:
                ...
                if not (conf_file := SDK_ROOT_DIR / ext_arg.replace('-DCONF_FILE=', '')).exists():
                    ...
                    continue
                custom_conf_files.append(conf_file)
                new_args.append(f'-DCONF_FILE={conf_file.name}')
            example_yml[k]['contents']['document']['extra_build_args'] = new_args

        # For examples/ota_examples/ota_mcuboot_client/enet
        if not hasattr(self, 'app_name'):
            self.app_name = next(iter(example_yml))[0]

        return example_yml, custom_conf_files, extra_files, replacements

    def build_cmd(self, extra_args=[], build_dir='build', repo=False):
        board_var = self.options.cmake_variables.get('board', "<board_id>")
        if repo:
    ...
    def copy_extra_files(self, extra_files):
        current_board = self.options.cmake_variables.get('board')
        for entry in extra_files:
            if isinstance(entry, str):
                src_path = self._replace_cmake_variables(entry)
                if not src_path:
                ...
                if should_skip_entry(entry, current_board):
                    continue

                src_path = self._replace_cmake_variables(entry.get('source', ''))
                if not src_path:
                    logger.warning(f"[extra_files] Empty source path in entry: {entry}")
                    continue
    ...
    def combine_prj_conf(self):
        ...
        '''
        example_root_stem = (SDK_ROOT_DIR / self.source_dir.relative_to(SDK_ROOT_DIR).parts[0]).stem
    ...
    def cm_externalzephyrproject_add(self, func: dict, o_func: CMakeFunction) -> dict:
        ...
        linked_app_options = AppOptions(
            source_dir=linked_source,
            output_dir=self.shared_options.output_dir / linked_source.name,
            app_type=AppType.linked_app,
            cmake_variables=self.shared_options.cmake_variables
        )
        self.linked_app = CmakeApp(self.shared_options, linked_app_options)
        self.linked_app.run()
    ...
    def cm_include(self, func: dict, o_func: CMakeFunction) -> dict:
        ...
        if not self.current_process_file.name == 'sysbuild.cmake':
            return o_func
        inc_path = self._replace_cmake_variables(inc_path)
    ...
    def replace_var(self, var, bypass_list=[]):
        for k, v in self.options.cmake_variables.items():
            if k in bypass_list:
                continue
            cmake_var = f"${{{k}}}"
    ...
    def _process_path(self, base_path='', src=''):
        return process_path(self.source_dir, base_path, src, self.options.cmake_variables)

    def _replace_cmake_variables(self, ori_str=''):
        return replace_cmake_variables(ori_str, self.options.cmake_variables)
    ...
    def apply_replacements(self, replacements):
        ...
        for replacement in replacements:
            if should_skip_entry(replacement, current_board):
                continue
