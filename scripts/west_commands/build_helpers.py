...
def find_build_dir(dir, guess=False, **kwargs):
    ...
    if dir:
        ...
        default_build_root = config.get('build', 'path', fallback=None)
        if isinstance(default_build_root, str) and not (_p := Path(default_build_root)).is_absolute():
            default_build_root = (Path(script_dir).parent.parent / _p).as_posix()
        cwd = os.getcwd()
        ...
        if default and is_mcux_build(default):
            build_dir = default
        elif is_mcux_build(cwd):
            build_dir = cwd
        else:
            build_dir = os.path.join(default_build_root, default) if default_build_root else default
...
def is_mcux_build(path):
    ...
    if 'SdkRootDirPath' in cache:
        log.dbg(f'{path} is a mcux sdk build directory',
                level=log.VERBOSE_EXTREME)
        return True

    log.dbg(f'{path} is NOT a valid mcux sdk build directory',
            level=log.VERBOSE_EXTREME)
    return False
