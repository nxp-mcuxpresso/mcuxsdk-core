...
class Kconfig(object):
    ...
    def _init(self, filename, warn, warn_to_stderr, encoding, enable_all_drivers):
        ...
        self.top_node.headfile = _EXAMPLE_CONFIG_HEAD
    ...
    def defconfig_filename(self):
        ...
        return None
    def _get_copyright_header(self):
        with open(_COPYRIGHT_HEADER_FILE, 'r') as file:
            # Read the content of the file
            self._copyright_header = file.read()
    ...
    def write_splitconf(self, header_dir=None, header=None):
        if header_dir:
            if not os.path.exists(header_dir):
                os.makedirs(header_dir)

        header_content = self._splitconf_contents(header)

        for filename, contents in header_content.items():
            filename = os.path.join(header_dir, filename)
            if len(contents) == 0 and _EXAMPLE_CONFIG_HEAD not in filename:
                continue
            # add header in the start of contents
            contents.insert(0, self._copyright_header)
            self.generated_headers.append(filename)
            if self._write_if_changed(filename, "".join(contents)):
    ...
    def _splitconf_contents(self, header):
        header_contents = {}
        ...
        for sym in self.unique_defined_syms:
            if len(sym.nodes) > 1:
                for node in sym.nodes:
                    if node.headfile not in header_contents:
                        header_contents[node.headfile] = [header] if header else []
                    self._format_sym_to_define(sym, header_contents[node.headfile])
            else:
                if sym.nodes[0].headfile not in header_contents:
                    header_contents[sym.nodes[0].headfile] = [header] if header else []
                self._format_sym_to_define(sym, header_contents[sym.nodes[0].headfile])

        # add include guard to each header of header_contents
        self._add_include_guard_to_header(header_contents)

        return header_contents

    def _add_include_guard_to_header(self, header_contents):
        for filename, contents in header_contents.items():
            if len(contents) == 0:
                continue
            macro_name = "_" + ("_").join(filename.upper().split('.')) + "_"
            include_macro_ifndef = "#ifndef " + macro_name + "\n"
            include_macro_define = "#define " + macro_name + "\n\n"
            include_macro_endif = "\n#endif /* " + macro_name + " */\n"
            contents.insert(0, include_macro_ifndef)
            contents.insert(1, include_macro_define)
            contents.append(include_macro_endif)
    ...
    def _autoconf_contents(self, header):
        ...
        for sym in self.unique_defined_syms:
            ...
            self._format_sym_to_define(sym, chunks)
    ...
    def _match_head(self, prompt):
        match = _head_file_match(prompt)
        if match:
            return match.group(1)
        else:
            return False

    def _format_sym_to_define(self, sym, list):
        val = sym.str_value
        if not sym._write_to_conf:
            return

        if self._mcux_misc_macro(sym.name):
            return
        ...
        if sym.orig_type in _BOOL_TRISTATE:
            if val == "y":
                ...
                _macro = "// #define {}{} 0".format(updated_config_prefix, sym.name)
            elif val == "m":
                _macro = "#define {}{}_MODULE 1".format(updated_config_prefix, sym.name)
            ...
            if sym.orig_type is HEX and \
                ...
                val = "0x" + val
            _macro = "#define {}{} {}".format(updated_config_prefix, sym.name, val)
    ...
    def _parse_block(self, end_token, parent, prev):
        ...
        while self._next_line():
            ...
            if t0 in [_T_CONFIG, _T_MENUCONFIG, _T_CONFIGDEFAULT]:
                ...
                headfile = self._match_head(node.prompt[0])
                if headfile:
                    node.headfile = headfile
                else:
                    node.headfile = parent.headfile
...
# Felix: The second part is for macro arguments like (x,y)
_id_keyword_match = _re_match(r"([A-Za-z0-9_$/.-]+(\([,\sA-Za-z0-9_]+\))?)\s*")
...
_head_file_match = _re_match(r".*\((\w+\.h)\)")