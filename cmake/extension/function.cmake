...
function(mcux_add_codewarrior_lib_path)
  set(multi_value TARGETS LIB_PATH)
  cmake_parse_arguments(_ "${options}" "${single_value}" "${multi_value}"
          ${ARGN})
  ...
  if(${CONFIG_TOOLCHAIN} STREQUAL "codewarrior")
    ...
    if(__LIB_PATH)
      foreach(item ${__LIB_PATH})
        mcux_append_single_compiler_flags_variable(LD "-L${item}")
      endforeach()
    endif ()
  endif()
endfunction()
...
function(_mcux_filter_iar_pa217_flag output_var flag)
  set(filtered_flag "${flag}")

  if(NOT CONFIG_TOOLCHAIN STREQUAL "iar")
    set(${output_var} "${filtered_flag}" PARENT_SCOPE)
    return()
  endif()
  ...
  if(NOT "${CMAKE_C_COMPILER_VERSION}" VERSION_LESS "9.70.4")
    set(${output_var} "${filtered_flag}" PARENT_SCOPE)
    return()
  endif()

  if(NOT "${flag}" MATCHES "^--diag_suppress=")
    set(${output_var} "${filtered_flag}" PARENT_SCOPE)
    return()
  endif()

  string(REGEX REPLACE "^--diag_suppress=" "" diag_list "${flag}")
  string(REPLACE "," ";" diag_items "${diag_list}")
  list(FIND diag_items "Pa217" pa217_index)

  if(pa217_index EQUAL -1)
    set(${output_var} "${filtered_flag}" PARENT_SCOPE)
    return()
  endif()

  list(REMOVE_ITEM diag_items "Pa217")
  if(diag_items)
    list(JOIN diag_items "," filtered_diags)
    set(filtered_flag "--diag_suppress=${filtered_diags}")
  else()
    set(filtered_flag "")
  endif()

  log_debug(
    "Skip unsupported IAR diagnostic suppression Pa217 for compiler version ${CMAKE_C_COMPILER_VERSION}"
    ${CMAKE_CURRENT_LIST_FILE})
  set(${output_var} "${filtered_flag}" PARENT_SCOPE)
endfunction()

function(_mcux_filter_iar_pa217_configs output_var)
  set(filtered_configs)

  foreach(item ${ARGN})
    _mcux_filter_iar_pa217_flag(filtered_item "${item}")
    if(filtered_item)
      list(APPEND filtered_configs "${filtered_item}")
    endif()
  endforeach()

  set(${output_var} ${filtered_configs} PARENT_SCOPE)
endfunction()
...
function(mcux_add_configuration)
  ...
  set(filtered_as ${__AS})
  set(filtered_cc ${__CC})
  set(filtered_cx ${__CX})
  set(filtered_ld ${__LD})
  if(${CONFIG_TOOLCHAIN} STREQUAL "iar")
    _mcux_filter_iar_pa217_configs(filtered_as ${filtered_as})
    _mcux_filter_iar_pa217_configs(filtered_cc ${filtered_cc})
    _mcux_filter_iar_pa217_configs(filtered_cx ${filtered_cx})
    _mcux_filter_iar_pa217_configs(filtered_ld ${filtered_ld})
  endif()
  ...
  if(__TARGETS)
    foreach(item ${__TARGETS})
      if(${item} STREQUAL ${CMAKE_BUILD_TYPE})
        if(filtered_as)
          _mcux_add_configuration(AS ${filtered_as})
        endif()
        if(filtered_cc)
          _mcux_add_configuration(CC ${filtered_cc})
        endif()
        if(filtered_cx)
          _mcux_add_configuration(CX ${filtered_cx})
        endif()
        if(filtered_ld)
          _mcux_add_configuration(LD ${filtered_ld})
        endif()
        if(__LIB)
...
function(mcux_remove_configuration)
  ...
  if(__TARGETS)
    foreach(item ${__TARGETS})
      if(${item} STREQUAL ${CMAKE_BUILD_TYPE})
        if(filtered_as)
          _mcux_remove_configuration(AS ${filtered_as})
        endif()
        if(filtered_cc)
          _mcux_remove_configuration(CC ${filtered_cc})
        endif()
        if(filtered_cx)
          _mcux_remove_configuration(CX ${filtered_cx})
        endif()
        if(filtered_ld)
          _mcux_remove_configuration(LD ${filtered_ld})
        endif()
      endif()
    endforeach()
  else()
