...
function(ExternalZephyrProject_Add)
  ...
  cmake_parse_arguments(ZBUILD "" "APPLICATION;board;board_REVISION;core_id;config;toolchain;SOURCE_DIR;APP_TYPE;BUILD_ONLY" "${multi_value}" ${ARGN})

  # Felix: if user does not specify board, use default
  set_ifndef(ZBUILD_board ${board})
  set_ifndef(ZBUILD_toolchain ${CONFIG_TOOLCHAIN})
  set_ifndef(ZBUILD_config ${CMAKE_BUILD_TYPE})
#  if(NOT DEFINED ZBUILD_board)
#    set(ZBUILD_board ${board})
#  endif ()
  ...
  if(ZBUILD_CMAKE_ARGS)
    set(USER_DEFINED_CMAKE_ARGS ${ZBUILD_CMAKE_ARGS})
  else()
    set(USER_DEFINED_CMAKE_ARGS)
  endif ()
  ...
  ExternalProject_Add(
    ...
    CMAKE_ARGS -DSYSBUILD:BOOL=True
               ...
               ${shared_cmake_vars_argument} ${USER_DEFINED_CMAKE_ARGS}
  ...
  if(DEFINED ZBUILD_core_id)
    set_target_properties(${ZBUILD_APPLICATION} PROPERTIES core_id ${ZBUILD_core_id})
  endif()

  if(DEFINED ZBUILD_config)
    set_target_properties(${ZBUILD_APPLICATION} PROPERTIES config ${ZBUILD_config})
  endif()

  if(DEFINED ZBUILD_toolchain)
    set_target_properties(${ZBUILD_APPLICATION} PROPERTIES toolchain ${ZBUILD_toolchain})
  endif()
...
# This function includes a meta build system into the multiimage build system
#
# APPLICATION: <name>:       Name of the application, name will also be used for build folder of the application
# SOURCE_DIR <dir>:          Source directory of the application
# board <board>:             Use <board> for application build, if not set,use board from command line parameter
# core_id <core_id>:             Use <core_id> for application build, if not set, use core_id from command line parameter
# config <config>:             Use <config> for application build, if not set, use config from command line parameter
# toolchain <toolchain>:             Use <toolchain> for application build, if not set, use toolchain from command line parameter
# Note: All parameters can be configured by Kconfig.sysbuild or command line variable with "-D" prefix
function(ExternalMCUXProject_Add)
  set(single_value APPLICATION SOURCE_DIR board core_id config toolchain)
...
function(ExternalZephyrProject_Cmake)
  ...
  get_target_property(${ZCMAKE_APPLICATION}_toolchain      ${ZCMAKE_APPLICATION} toolchain)
  get_target_property(${ZCMAKE_APPLICATION}_core_id     ${ZCMAKE_APPLICATION} core_id)
  get_target_property(${ZCMAKE_APPLICATION}_config     ${ZCMAKE_APPLICATION} config)
  get_target_property(${ZCMAKE_APPLICATION}_device     ${ZCMAKE_APPLICATION} device)

  if (NOT ${ZCMAKE_APPLICATION}_board)
    set(${ZCMAKE_APPLICATION}_board ${board})
  endif ()

  if (NOT ${ZCMAKE_APPLICATION}_toolchain)
    set(${ZCMAKE_APPLICATION}_toolchain ${toolchain})
  endif ()

  if (NOT ${ZCMAKE_APPLICATION}_core_id)
    set(${ZCMAKE_APPLICATION}_core_id ${core_id})
  endif ()

  if (NOT ${ZCMAKE_APPLICATION}_config)
    set(${ZCMAKE_APPLICATION}_config ${config})
  endif ()

  if (NOT ${ZCMAKE_APPLICATION}_device)
    set(${ZCMAKE_APPLICATION}_device ${device})
  endif ()
  ...
  if (${ZCMAKE_APPLICATION}_board)
    list(APPEND cmake_extra_args "-Dboard=${${ZCMAKE_APPLICATION}_board}")
  endif ()
  if (${ZCMAKE_APPLICATION}_device)
    list(APPEND cmake_extra_args "-Ddevice=${${ZCMAKE_APPLICATION}_device}")
  endif ()
  if (${ZCMAKE_APPLICATION}_core_id)
    list(APPEND cmake_extra_args "-Dcore_id=${${ZCMAKE_APPLICATION}_core_id}")
  endif ()
  if (${ZCMAKE_APPLICATION}_config)
    list(APPEND cmake_extra_args "-DCMAKE_BUILD_TYPE=${${ZCMAKE_APPLICATION}_config}")
  endif ()
  ...
  if (${${ZCMAKE_APPLICATION}_toolchain} MATCHES "iar|mdk|xtensa|armgcc" AND FOUND_RUBY_EXECUTABLE)
    ...
    if(return_val)
      message(WARNING
              "CMake configure failed for GUI project generation: ${ZCMAKE_APPLICATION}\n"
              "Location: ${SOURCE_DIR}"
              )
    endif()
  endif ()
